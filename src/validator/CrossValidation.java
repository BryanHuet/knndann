...
public class CrossValidation {
    ...
    public void crossValidation() {
        ...
        for (int p : parameters) {
            for (Fold fold : this.folds) {
                DANN dann = new DANN(new HashSet<>(fold.getTrain_set()), p, NB_ITERATION, parametersNumber, classNumber);
                for (Element query : fold.getValidation_set()) {
                    query.setPredict(dann.proceed(query));
                }
                double score = this.score(fold.getValidation_set());
                fold.setScore(score);
                scores += fold.getScore();
                if (score > bestScore) {
                    bestScore = score;
                    this.best_training_set = fold.getTrain_set();
                }
            }
     ...
     * @param path chemin de fichier de data. Permet d'afficher ou non le nom de fichier.
     */
